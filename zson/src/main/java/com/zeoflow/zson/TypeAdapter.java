 ...
 * By default Zson converts application classes to JSON using its built-in type
 * adapters. If Zson's default JSON conversion isn't appropriate for a type,
 ...
 *     }
 *   }}</pre>
 * With this type adapter installed, Zson will convert {@code Points} to JSON as
 ...
 * registering the type adapter with Zson. If your {@code Zson} instance
 * has been configured to {@link ZsonBuilder#serializeNulls()}, these nulls will be
 * written to the final document. Otherwise the value (and the corresponding name
 * when writing to a JSON object) will be omitted automatically. In either case
 * your type adapter must handle null.
 *
 * <p>To use a custom type adapter with Zson, you must <i>register</i> it with a
 * {@link ZsonBuilder}: <pre>   {@code
 *
...
// <p>A type adapter registered with Zson is automatically invoked while serializing
// or deserializing JSON. However, you can also use type adapters directly to serialize
// and deserialize JSON. Here is an example for deserialization: <pre>   {@code
//
//   String json = "{'origin':'0,0','points':['1,2','3,4']}";
//   TypeAdapter<Graph> graphAdapter = Zson.getAdapter(Graph.class);
...
public abstract class TypeAdapter<T> {
   ...
   * Unlike Zson's similar {@link Zson#toJson(JsonElement, Appendable) toJson}
   ...
   * <pre>   {@code
   *
   * Zson Zson = new ZsonBuilder().registerTypeAdapter(Foo.class,
   *   new TypeAdapter<Foo>() {
  ...
  /**
   * Converts {@code value} to a JSON document. Unlike Zson's similar {@link
   * Zson#toJson(Object) toJson} method, this write is strict. Create a {@link
   * JsonWriter#setLenient(boolean) lenient} {@code JsonWriter} and call
   * {@link #write(JsonWriter, Object)} for lenient
   * writing.
   *
  ...
  /**
   * Converts the JSON document in {@code in} to a Java object. Unlike Zson's
   * similar {@link Zson#fromJson(java.io.Reader, Class) fromJson} method, this
  ...
  /**
   * Converts the JSON document in {@code json} to a Java object. Unlike Zson's
   * similar {@link Zson#fromJson(String, Class) fromJson} method, this read is
