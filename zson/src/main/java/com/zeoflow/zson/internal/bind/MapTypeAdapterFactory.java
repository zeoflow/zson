...
import com.zeoflow.zson.internal.JsonReaderInternalAccess;
import com.zeoflow.zson.internal.ObjectConstructor;
 ...
 *   }
 * }</pre>
 * But Zson is unable to deserialize this value because the JSON string name is
 * just the {@link Object#toString() toString()} of the map key. Attempting to
 * convert the above JSON to an object fails with a parse exception:
 * <pre>com.google.Zson.JsonParseException: Expecting object found: "(5,6)"
 *   at com.google.Zson.JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at com.google.Zson.ObjectNavigator.navigateClassFields
 *   ...</pre>
 *
 ...
 *
 * <p>Register this adapter when you are creating your Zson instance.
 * <pre>   {@code
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
  ...
  @Override public <T> TypeAdapter<T> create(Zson zson, TypeToken<T> typeToken) {
    ...
    Class<?> rawTypeOfSrc = $Zson$Types.getRawType(type);
    Type[] keyAndValueTypes = $Zson$Types.getMapKeyAndValueTypes(type, rawTypeOfSrc);
    TypeAdapter<?> keyAdapter = getKeyAdapter(zson, keyAndValueTypes[0]);
    TypeAdapter<?> valueAdapter = zson.getAdapter(TypeToken.get(keyAndValueTypes[1]));
    ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

    @SuppressWarnings({"unchecked", "rawtypes"})
    // we don't define a type parameter for the key or value types
    TypeAdapter<T> result = new Adapter(zson, keyAndValueTypes[0], keyAdapter,
   ...
   */
  private TypeAdapter<?> getKeyAdapter(Zson context, Type keyType) {
  ...
  private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
    ...
    public Adapter(Zson context, Type keyType, TypeAdapter<K> keyTypeAdapter,
