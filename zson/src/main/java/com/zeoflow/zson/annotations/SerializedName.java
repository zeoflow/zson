 ...
 * the default field naming policy, that may have been set on the {@link Zson}
 * instance. A different naming policy can set using the {@code ZsonBuilder} class. See
 * {@link ZsonBuilder#setFieldNamingPolicy(FieldNamingPolicy)}
 ...
 * For example:
 * <pre>
 *   MyClass target = Zson.fromJson("{'name1':'v1'}", MyClass.class);
 *   assertEquals("v1", target.b);
 *   target = Zson.fromJson("{'name2':'v2'}", MyClass.class);
 *   assertEquals("v2", target.b);
 *   target = Zson.fromJson("{'name3':'v3'}", MyClass.class);
