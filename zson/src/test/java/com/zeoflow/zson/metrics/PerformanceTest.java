...
package com.zeoflow.zson.metrics;
...
/**
 * Tests to measure performance for Zson. All tests in this file will be disabled in code. To run
...
public class PerformanceTest extends TestCase {
  ...
  public void disabled_testStringDeserialization() {
    ...
    while (true) {
      try {
        ...
        parseLongJson(json);
        System.out.println("Zson could handle a string of size: " + stackTrace.length());
  ...
  private void parseLongJson(String json) throws JsonParseException {
    ExceptionHolder target = zson.fromJson(json, ExceptionHolder.class);
  ...
  /**
   * Created in response to http://code.google.com/p/google-Zson/issues/detail?id=96
   */
  ...
  public void disabled_testLargeCollectionDeserialization() {
    ...
    List<CollectionEntry> list = zson.fromJson(json, collectionType);
   ...
   * Created in response to http://code.google.com/p/google-Zson/issues/detail?id=96
   */
  // Last I tested, Zson was able to serialize upto 14MB byte array
  public void disabled_testByteArraySerialization() {
    for (int size = 4145152; true; size += 1036288) {
      ...
      zson.toJson(ba);
      System.out.printf("Zson could serialize a byte array of size: %d\n", size);
    }
  }
   ...
   * Created in response to http://code.google.com/p/google-Zson/issues/detail?id=96
   */
  // Last I tested, Zson was able to deserialize a byte array of 11MB
  public void disable_testByteArrayDeserialization() {
    for (int numElements = 10639296; true; numElements += 16384) {
      ...
      sb.append("]");
      String json = sb.toString();
      byte[] ba = zson.fromJson(json, byte[].class);
      System.out.printf("Zson could deserialize a byte array of size: %d\n", ba.length);
    }
  }

// The tests to measure serialization and deserialization performance of Zson
// Based on the discussion at
// http://groups.google.com/group/google-Zson/browse_thread/thread/7a50b17a390dfaeb
...
// Deserialized exposed classes avg time: 173 ms
  ...
  public void disabled_testDeserializeClasses() {
    ...
    long t1 = System.currentTimeMillis(); 
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
      target[i] = zson.fromJson(json, ClassWithList.class);
    }
    long t2 = System.currentTimeMillis(); 
    long avg = (t2 - t1) / NUM_ITERATIONS;
  ...
  public void disable_testLargeObjectSerializationAndDeserialization() {
    ...
    long t1 = System.currentTimeMillis(); 
    String json = zson.toJson(largeObject);
    long t2 = System.currentTimeMillis();
    System.out.printf("Large object serialized in: %d ms\n", (t2 - t1));

    t1 = System.currentTimeMillis(); 
    zson.fromJson(json, new TypeToken<Map<String, Long>>() {}.getType());
  ...
  public void disabled_testDeserializeExposedClasses() {
    ...
    long t1 = System.currentTimeMillis(); 
    for (int i = 0; i < NUM_ITERATIONS; ++i) {
      target[i] = zson.fromJson(json, ClassWithListOfObjects.class);
    }
    long t2 = System.currentTimeMillis(); 
    long avg = (t2 - t1) / NUM_ITERATIONS;
  ...
  public void disabled_testLargeZsonMapRoundTrip() throws Exception {
    ...
    Zson zson = new Zson();
    String json = zson.toJson(original);
    Type longToLong = new TypeToken<Map<Long, Long>>(){}.getType();
    zson.fromJson(json, longToLong);
  }
