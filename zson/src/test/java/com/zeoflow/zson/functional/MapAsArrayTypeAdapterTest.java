...
public class MapAsArrayTypeAdapterTest extends TestCase {
  ...
  public void testSerializeComplexMapWithTypeAdapter() {
    ...
    String json = zson.toJson(original, type);
    assertEquals("[[{\"x\":5,\"y\":5},\"a\"],[{\"x\":8,\"y\":8},\"b\"]]", json);
    assertEquals(original, zson.<Map<Point, String>>fromJson(json, type));
    ...
    assertEquals("{\"t\":true,\"f\":false}",
        zson.toJson(otherMap, Map.class));
    assertEquals("{\"t\":true,\"f\":false}",
        zson.toJson(otherMap, new TypeToken<Map<String, Boolean>>() {}.getType()));
    assertEquals(otherMap, zson.<Object>fromJson("{\"t\":true,\"f\":false}",
  ...
  public void disabled_testTwoTypesCollapseToOneSerialize() {
    ...
    original.put(1.0F, "b");
    try {
      zson.toJson(original, new TypeToken<Map<Number, String>>() {}.getType());
  ...
  public void testTwoTypesCollapseToOneDeserialize() {
    ...
    try {
      zson.fromJson(s, new TypeToken<Map<Double, String>>() {}.getType());
      fail();
    } catch (JsonSyntaxException expected) {
    }
  }
  ...
  public void testMapWithTypeVariableSerialization() {
    Zson zson = new ZsonBuilder().enableComplexMapKeySerialization().create();
  ...
  public void testMapWithTypeVariableDeserialization() {
    ...
    Type type = new TypeToken<PointWithProperty<Point>>(){}.getType();
    PointWithProperty<Point> map = zson.fromJson(json, type);
