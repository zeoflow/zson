...
public final class RuntimeTypeAdapterFactoryFunctionalTest extends TestCase {
   ...
   * work correctly for {@link Zson#getDelegateAdapter(TypeAdapterFactory, TypeToken)}.
   */
  public void testSubclassesAutomaticallySerialized() throws Exception {
    Shape shape = new Circle(25);
    String json = zson.toJson(shape);
    shape = zson.fromJson(json, Shape.class);
    assertEquals(25, ((Circle)shape).radius);

    shape = new Square(15);
    json = zson.toJson(shape);
    shape = zson.fromJson(json, Shape.class);
  ...
  static class RuntimeTypeAdapterFactory<T> implements TypeAdapterFactory {
    ...
    @Override public <R> TypeAdapter<R> create(Zson zson, TypeToken<R> type) {
      ...
      for (Map.Entry<String, Class<?>> entry : labelToSubtype.entrySet()) {
        TypeAdapter<?> delegate = zson.getDelegateAdapter(this, TypeToken.get(entry.getValue()));
